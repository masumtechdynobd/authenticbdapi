...
export default function Checkout({ total_point, data, addressList }: any) {
    ...
    const { setCartData, cartData, totalQuantity, temp_user_id } = useCartStoreData();
        ...
        <Container>
            {
                cartData && cartData.length > 0 ?
                        ...
                        <div className="p-6 bg-white shadow-md rounded-md">
                            <div className='flex flex-wrap justify-between '>
                                <h2 className="text-lg font-semibold mb-4">Delivery Address</h2>
                                {
                                    ...
                                    <div>
                                        <Button onClick={() => setIsAddressModal(!isAddressModal)} >Use Other Address</Button>
                                    </div>
                                }
                                {
                                    session?.data?.user ===null ?"" :
                                <Link className='uppercase bg-primary px-4 rounded-lg py-2 flex items-center justify-center text-white ' href="/login">Login</Link>
                                } 
                            </div>
                            <form onSubmit={handleSubmit}>
                                <AddressForm errors={errors} setErrors={setErrors} formData={formData} setFormData={setFormData} country={country} state={state} cities={cities} />
                                            ...
                                            <div className='relative'>
                                                <span>৳{total_point}</span>
                                                {
                                                    summary?.point_applied ? <span onClick={pointRemove} className="text-sm left-[-15px] py-[1px] px-[6px] bottom-[-18px] absolute bg-red-600 cursor-pointer text-white font-semibold  rounded-lg shadow-md hover:bg-red-900 transition duration-300">
                                                        Remove
                                                    </span> : <span onClick={pointApply} className=" text-sm left-[-30px] py-[1px] px-[6px] bottom-[-18px] absolute bg-accent-lightPink cursor-pointer text-white font-semibold  rounded-lg shadow-md hover:bg-purple-700 transition duration-300">
                                                        Redeem
                                                    </span>
                                                }
                    ...
                    : <div className='flex flex-col items-center justify-center py-10 rounded-lg ' >
                        ...
                        <Link className='text-primary underline ' href={"/"}>RETURN TO SHOP</Link>
                    </div>}
                    ...
                    <DialogOverlay className='z-[9999999] '>
                        ...
                        <DialogContent className=" bottom-0 left-0 bg-transparent flex items-center justify-center z-[99999999]" >
                            <div className='bg-white relative rounded-lg  '>
                                <div onClick={() => setIsAddressModal(false)} className=' cursor-pointer absolute top-2 right-3 ' >
...

                                        {
                                            addressList?.map((item: any) => (
                                                ...
                                                <div className="col-span-1 lg:col-span-1  " key={item.id} >
                                                    ...
                                                    }} className={` cursor-pointer border p-3 pr-5 rounded mb-3 relative text-left ${selectedAddress?.id === item.id ? 'border-primary' : ''} `}>
                                                        <div>
