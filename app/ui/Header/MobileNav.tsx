...
import { ChevronLeft, ChevronRight, CircleUserRound, Heart, Languages, Menu, ShoppingCart, User, X } from 'lucide-react';
...
const MobileNav = ({
    ...
    return (
        <>
            <div className={`block md:hidden sticky top-0 z-[999999] bg-white py-2  ${isSticky ? 'py-4 bg-white shadow-md' : ''}`}>
                    ...
                    <div className='flex items-center gap-[20px]'>
                        ...
                        <div className="shoppingCart relative pr-3 ">
                            <Dialog open={openCart} onOpenChange={setOpenCart} >
                                <DialogTrigger>
                                    <div className="flex items-center gap-1 text-base capitalize" onClick={() => setOpenCart(!openCart)} >
                                        <CartIcon />
                                    </div>
                    ...
                    <div className={`b-menu_subpanel m-active_level_${labelMenu}`}>
                        ...
                        <div className="b-menu_subpanel-container m-level_1">
                            <div className="b-menu_panel-head">
                                ...
                                <CircleUserRound />
                            <Link href={"/login"} onClick={handCloseMenu}>Login</Link>
                                </div>
                            ...
                            <div className="tab_menu">
                                <div className="flex mb-4">
                                    <button
                                        ...
                                        className={`flex-1 py-3 text-sm font-medium relative ${activeTab === 'categories'
                                            ? 'text-primary'
                                            : 'text-gray-500'
                                            }`}
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`flex-1 py-3 text-sm font-medium relative ${activeTab === 'brands'
                                            ? 'text-primary'
                                            : 'text-gray-500'
                                            }`}
                                    >
                            ...
                            }
                            {
                                ...
                                <div className="b-menu_bar">
                                    <nav className="b-menu_bar-container">
                                        <BeautyApp handCloseMenu={handCloseMenu} menu_data={menu_data} />
                                        <div className="w-full  flex items-center justify-center gap-2 py-4 ">
                                            <div className="cursor-pointer  transition-all relative     duration-300 ease-in-out text-white" >
                                                <div className="flex flex-col max-w-max   transition-all bg-[#CC0F99] px-2 py-1 rounded-[30px] hover:scale-[1.1] ">
                                                    <Link onClick={handCloseMenu} href={`/blog`} className="uppercase text-white text-[12px] font-normal   "   >{blog}</Link>
                                                </div>
                                            </div>
                                            {
                                                offerMenu?.map((item: any, index: any) => (

                                                    <div key={index} className="key={item.id} cursor-pointer  transition-all relative     duration-300 ease-in-out text-white" >
                                                        <div className={`flex flex-col max-w-max  transition-all ${index === 0 ? 'bg-[#F0A401]' : 'bg-[#601390]'} px-2 py-1 rounded-[30px] hover:scale-[1.1] `}>
                                                            <Link onClick={handCloseMenu} href={`/category/${item.slug}`} className="uppercase text-white text-[10px] font-normal   "   >{item?.name} </Link>
                                                        </div>
                                                    </div>
                                                ))
                                            }
                                        ...
                                        <div>
                                            ...
                                            {
                                                cookieValue?.user ? <>
                                                    <AccountMenu sign_out={<SignOutForm sign_out={"Sign out"} />} profile_data={profile_data} />
