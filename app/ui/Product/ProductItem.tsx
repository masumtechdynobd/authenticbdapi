...
const ProductItem: React.FC<Product> = ({ id, discount_text, bestseller, best_seller_text, free_shipping_text, freeshipping, colors, choice_options, stocks, name, slug, brand, thumbnail_image, discount, rating, totalRating, stroked_price, main_price }) => {
  return (
    <div className="p-1">
      <div className="product_item relative bg-white border px-2 py-2  group overflow-hidden rounded-lg">
        {/* free shipping start */}
        {
          ...
          <div className={`flex flex-col gap-1 absolute overflow-hidden bg-neutral-black rounded-sm z-10 ${bestseller > 0 ? 'top-[27px] md:top-[35px]' : ''}  `}
          >
            <div className="flex flex-row gap-1 rounded-sm px-2 py-0.5 items-center"
            >
              <p className="text-[10px] font-semibold uppercase text-white">{free_shipping_text}</p>
            </div>
          </div>
        }
        {/* free shipping end */}
        {/* free shipping start */}
        {
          ...
          <div className="flex flex-col gap-1 absolute overflow-hidden  rounded-sm z-10 "
            style={{
              backgroundImage: "linear-gradient(90deg, #efb5df 0%, #9f1fef 50.52%, rgb(180 122 217) 100%)",
              backgroundColor: "initial"
            }}
          >
            <div className="flex flex-row gap-1 rounded-sm px-2 py-0.5 items-center"
            >
              <p className="text-[10px] font-semibold uppercase text-white">{best_seller_text}</p>
            </div>
          </div>
        }
        {/* free shipping end */}
        ...
        <Link href={`/product/${slug}`} >
          <div className="tum_image   flex relative items-center justify-center flex-col overflow-hidden "
          ...
          <h3 className="text-[12px] sm:text-base text-neutral-black  font-medium line-clamp-2  ">
            <Link href={`/product/${slug}`} > {name}</Link>
          </h3>

          <div className="price  flex items-center gap-2 justify-start flex-wrap ">
            ...
            {
              discount > 0 && <div className="offer max-w-max  text-[12px] sm:text-[12px] ">
                <span>{discount}{discount_text} </span>
              </div>
            }

          </div>
          <SingleAddToCart isShow={true} look={false} id={id} stocks={stocks} choice_options={choice_options} colors={colors} thumbnail_image={thumbnail_image} name={name} stroked_price={stroked_price} main_price={main_price} totalRating={totalRating} brand={brand} discount={discount} />
          </div>
        </div>
      </div>
    </div>
