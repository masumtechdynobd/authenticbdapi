            ...
            <div className='flex justify-between flex-row gap-3 w-full '>
                <div className=' w-full flex items-center  md:max-w-max gap-3 ' >
                    ...
                    <div className="qty_input_area flex items-center w-1/2 md:max-w-max flex-1  ">
                        <Button onClick={decreaseQty} className='w-[40px] h-[40px] !rounded-l-[5px]  !bg-p_light !rounded-[0px] hover:text-white !p-0 flex items-center justify-center transition-all  duration-300 ease-in-out'  >
                            <MinusIcon className="!text-primary hover:!text-white  transition-all  duration-300 ease-in-out" />
                        </Button>
                        <input onChange={(e) => setQty(Number(e.target.value))} value={qty} type="text" min={1} className='w-[40px] h-[40px] bg-primary text-center text-white ' />
                        <Button onClick={increaseQty} className='w-[40px] h-[40px] !rounded-r-[5px]  !bg-p_light !rounded-[0px] hover:text-white !p-0 flex items-center justify-center transition-all  duration-300 ease-in-out'  >
                ...
                <div className=' w-full  flex flex-wrap justify-end gap-4 '>
                    ...
                    <div className='mobile_cart'>
                        <Button className=' lg:max-w-max px-[20px] justify-center' disabled={loading} onClick={addToCartHandler} >
                            ADD TO CART {loading && <Loader2 className="animate-spin h-5 w-5 text-white" />}
                        </Button>
                    </div>
                    {
                        sizeChart &&
                        ...
                        <div className='flex justify-between items-center '>
                            <div onClick={() => setIsSizeModal(true)} className=' cursor-pointer size_chart border-[1px] border-primary px-[6px] lg:px-[10px] py-[6px] text-center text-sm lg:text-lg  rounded-[30px] '>
                                {translateValue?.size_chart} </div>
                        </div>
                    }
                </div>
            </div>
