...
import { CategorySkeleton, ImageSkeleton, ProductSkeleton } from '@/app/ui/skeletons';
...
export default function ConcernCategoryProduct({ category }: ConcernCategoryProps) {
        ...
        <>
            ...
            <div className="category flex items-center justify-center flex-wrap gap-[6px] sm:gap-6 flex-row">
                <Suspense fallback={
                    ...
                    {category && category?.slice(0, 3).map((item) => (
                        <Button
                            onClick={() => {
                            ...
                            key={item.id}
                            className='inline-block py-[6px] sm:py-[10px] px-[30px] sm:px-[72px] border-primary border-[1px] rounded-[35px] transition duration-300 ease-in-out hover:bg-primary-hover hover:!text-white uppercase text-[12px] !text-neutral-black bg-transparent'
                        >
                            {item.name}
                        </Button>
                    ))}
                        ...
                        <CarouselContent className='ml-0'>
                            {loading
                                ? Array.from({ length: 4 }).map((_, index) => (
                                    <CarouselItem key={index} className="pl-0 lg:pl-2 basis-[50%] sm:basis-1/3 md:basis-1/3 xl:basis-1/5">
                                        <ProductSkeleton />
                                    </CarouselItem>
                                ))
